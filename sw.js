...
self.addEventListener('push', (event) => {
  let payload = {};
  try { payload = event.data ? event.data.json() : {}; } catch (e) { payload = { title: 'Шеф · Любимая', body: event.data ? event.data.text() : '' }; }
  event.waitUntil(
    self.registration.showNotification(payload.title || 'Шеф · Любимая', {
      body: payload.body || '',
      icon: 'icon-192.png',
      badge: 'icon-192.png',
      tag: 'shef-lubimaya',
